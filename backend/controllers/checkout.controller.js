  ...
  try {
    ...
    try {
      ...
      for (const it of items) {
        const p = await client.query("SELECT id, price_cents FROM products WHERE id = $1", [it.productId]);
        if (p.rowCount === 0) throw new Error("Producto no existe: " + it.productId);
        const unit = p.rows[0].price_cents;
        totalCents += unit * it.quantity;
        detailedItems.push({ productId: it.productId, quantity: it.quantity, unitPriceCents: unit });
      }

      // Aplicar cupón
      let couponId = null;
      if (coupon) {
        const c = await couponModel.findCouponByCode(coupon);
        if (!c) throw new Error("Cupón inválido");
        if (c.expires_at && new Date(c.expires_at) < new Date()) throw new Error("Cupón expirado");
        if (c.usage_limit && c.usage_count >= c.usage_limit) throw new Error("Cupón agotado");
        ...
        if (c.discount_percent) {
          totalCents = Math.max(0, Math.round(totalCents * (100 - c.discount_percent) / 100));
